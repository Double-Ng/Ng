    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("a.ReceivingGoods").click(function () {
                return confirm("您确定将这笔编号为：【" + $(this).parent().parent().find("td:first").text() + "】的订单收货吗？");
            });
...
<div id="header">
    ...
    <div>
        ...
        <a href="order/showAllOrders.do">注销</a>&nbsp;&nbsp;
        ...
        <c:if test="${not empty requestScope.user_orders}">
            <c:forEach items="${requestScope.user_orders}" var="user_orders">
                <tr>
                    ...
                    <td><a href="order/showOrderDetail.do?orderID=${user_orders.orderID}">查看详情</a></td>
                    <td>
                        <%--当订单状态为1（已发货）时，才可以点击确认收货--%>
                        <c:if test="${user_orders.status eq 1}">
                            <a class="ReceivingGoods"
                               href="order/receiverOrder.do?status=2&orderID=${user_orders.orderID}">确认收货</a>
                        </c:if>
                         <%--否则无法点击，显示对应状态信息。--%>
